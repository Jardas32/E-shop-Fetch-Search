...
const bgClear = document.querySelector('.btClear');
...
document.querySelector('.searchButton').addEventListener('click', () => {
   ...
   searchResult.classList.add('searchResultClass');
   searchResult.innerHTML = '';

   if (query.length === 0) {
      searchInput.value = 'Введите название товара';
      searchResult.classList.remove('searchResultClass');
      return;
   }
   ...
   if (foundProducts.length === 0) {
      searchInput.value = 'Упс... ничего не найдено.';
      searchResult.classList.remove('searchResultClass');
      return;
   }
...
document.addEventListener('click', (e) => {
    if(e.target.classList.contains('search-img')) {
      document.querySelector('.searchContainer').classList.toggle('searchContainerClass');
    }
})
...
function renderCart() {
   const cardStorage = JSON.parse(localStorage.getItem('carts')) || [];
   const countsCart = document.querySelector('.countsCart');
   countsCart.textContent = cardStorage.length;
   spanCounts.textContent = filteredProducts.length;

   block.innerHTML = '';

   currentProducts = paginateProducts(filteredProducts, currentPage, itemsPerPage);

   currentProducts.forEach((el, index) => {
      const globalIndex = (currentPage - 1) * itemsPerPage + index;

      const { id, image, title, price, quantity } = el;
      const normalPrice = price * quantity;
      const prices = normalPrice.toLocaleString('en-US', {
         style: 'currency',
         currency: 'USD',
         minimumFractionDigits: 0
      });
      ...
      card.setAttribute('class', 'card');
      card.innerHTML = `
         <button data-globalIndex="${index}" class="close">X</button>
         <div class="zoom">
            <img class="imgCard" src="${image}">
         </div>
         <p class="titleCard">${title}</p>
         <span class="priceCard">${prices}</span>
         <button data-globalIndex="${index}" class="closedCard">Add to cart</button>
      `;

      block.appendChild(card);
   });

   renderPaginationButtons(filteredProducts.length, itemsPerPage);
};
...
block.addEventListener('click', (e) => {
   ...
   // else if (e.target.classList.contains('btMinus')) {
   //    filteredProducts[globalIndex].quantity--;
   //    if (filteredProducts[globalIndex].quantity <= 0) {
   //       filteredProducts.splice(globalIndex, 1);
   //    }
   // } 
   else if (e.target.classList.contains('close')) {
      ...
      const card = e.target.closest('.card');
      const id = card.id;
      const imgas = card.querySelector('.imgCard').src;
      const titles = card.querySelector('.titleCard').textContent;
      const prices = card.querySelector('.priceCard').textContent;
      const price = parseInt(prices.replace(/\D/g, ''), 10);

      const newCard = { id, imgas, titles, price, quantity: 1 };

      const cardStorage = localStorage.getItem('carts') || "[]";
      const carts = JSON.parse(cardStorage);
...
openheader.addEventListener('click', () => {
    openheader.classList.toggle('open');
    document.querySelector('.bgsearch').classList.toggle('open');
});
